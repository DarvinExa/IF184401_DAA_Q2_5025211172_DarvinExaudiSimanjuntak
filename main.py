...
class Game():
    ...
    def canva(self):
        ...
        self.solveCan = None
        background_image = pg.image.load("testtest.jpg")
        self.canvas = pg.transform.scale(background_image, (402, 402))
        for i in range(21):
            pg.draw.line(self.canvas, "#000000", (i * 20, 0), (i * 20, 400), 2)
            pg.draw.line(self.canvas, "#000000", (0, i*20), (400, i*20), 2)
    ...
    def drawloop(self):
        ...
        if self.start is not None:
            node_img = pg.image.load("Idle.png")  
            node_img = pg.transform.scale(node_img, (20, 20))
            node_rect = node_img.get_rect()
            node_rect.center = ((self.start % self.cols) * 20 + 10, (self.start // self.cols) * 20 + 10)
            self.screen.blit(node_img, node_rect)
        if self.end is not None:
            node_img = pg.image.load("House.png") 
            node_img = pg.transform.scale(node_img, (20, 30))
            node_rect = node_img.get_rect()
            node_rect.center = ((self.end % self.cols) * 20 + 10, (self.end // self.cols) * 20 + 10)
            self.screen.blit(node_img, node_rect)
        pg.display.update()
    ...
    def find_path(self, start, end):
        ...
        for ind in path:
            rect = pg.Rect((int(ind)%20) * 20 + 5, (int(ind)//20) * 20 + 5, 10, 10)
            pg.draw.rect(self.solveCan, (255, 165, 0), rect)
            self.drawloop()  # Update the display after each node is drawn
            time.sleep(0.05)  # Delay for the path animation
        ...
        for child in children:
            if child not in closed:
                ...
                if abs(parent - child) == 1:
                    pg.draw.line(self.canvas, (139,179,18), ((col + 1 * (child > parent)) * 20, row*20 + 2),
                                 ((col + 1 * (child>parent)) * 20, (row+1)*20-1), 2)
                else:
                    pg.draw.line(self.canvas, (139,179,18), (col * 20 + 2, (row + 1 * (child > parent)) * 20),
